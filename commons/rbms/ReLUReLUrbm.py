# -*- coding: utf-8 -*-
"""
Created on Tue May 14 21:21:22 2019
...
#        return tf.nn.relu(tf.random_normal(shape = tf.shape(x), mean = x, stddev = sigma/3))
    ...
    def optimizer_fn(self):
        ...
        v_recon_act_clean = self.visible_activation(v_recon_mf, 0)

        
        positive_grad = self.calc_positive_gradient(self.h_act) #Hinton recomienda usar aquí las probabilidades en todo,                                                                   
        negative_grad = self.calc_negative_gradient(v_recon_act, h_recon_act)
        
        self.d_w = self.momentum_fn(self.d_w, self.lr*(self.calc_delta_w(positive_grad, negative_grad) - self.l1_ph*tf.abs(self.w)))
        self.d_vb = self.momentum_fn(self.d_vb, self.lr*self.calc_delta_visible_bias(v_recon_act)) #esto estaba con v_recon_mf_bias
        self.d_hb = self.momentum_fn(self.d_hb, self.lr*self.calc_delta_hidden_bias(self.h_act, h_recon_act))#Esto estaba con self.h_probs
