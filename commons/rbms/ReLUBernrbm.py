...
class ReLUBernRBM(RBM_base):
    ...
    def init_hidden_b(self):
        return tf.Variable(0.2*tf.ones([self.n_hidden]), dtype=tf.float32) #Hinton recomienda iniciarlos a 0 o a -4
...
#        h_act = tf.random_normal(tf.shape(h_prob), mean=h_prob, stddev=h_prob, dtype=tf.float32)
    ...
    def optimizer_fn(self):
        ...
        v_recon_clean = self.visible_activation(v_recon_mf, 0)
        
        positive_grad = self.calc_positive_gradient(self.h_prob) #Hinton recomienda usar aquí las probabilidades en todo,                                                                   
        negative_grad = self.calc_negative_gradient(v_recon_clean, h_recon_prob)
        
        self.d_w = self.momentum_fn(self.d_w, self.lr*(self.calc_delta_w(positive_grad, negative_grad) - self.l1_ph*tf.abs(self.w)))
        self.d_vb = self.momentum_fn(self.d_vb, self.lr*self.calc_delta_visible_bias(v_recon_clean)) #esto estaba con v_recon_mf_bias
